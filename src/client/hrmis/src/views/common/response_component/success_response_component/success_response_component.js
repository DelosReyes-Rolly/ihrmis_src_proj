        ...
        <React.Fragment>
            <div className="succeed-response">
                <div className="title-success-response">
                    <strong>{props.title}</strong> <span onClick={() => {dispatch(setSuccess(false))}}><MdClose size="14px"/></span>
                </div>
                <div className="children-success-response">
                    {props.children}              
                </div>
            </div>
        </React.Fragment>
...
SuccessResponseComponent.defaultProps = {
    children: 'Action succeed Lorem ip sum donor',
