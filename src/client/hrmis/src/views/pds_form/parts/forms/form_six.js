  ...
  const getDocs = async () => {
    ...
    await axios
      ...
      .then((response) => {
        ...
        data.forEach((data) => {
          let values = {
            filename: data.att_app_name,
            file: data.att_app_file,
            fileType: data.att_app_doc_id,
            id: data.att_id,
          };

          uploadedDocs.push(values);
        });
        if (!mounted.current) return;
      });
        ...
        <div className="pds-prof-class-one">
          <div>
            ...
            {dataListofObjectFiles.map((data) => {
              const docs = uploaded.filter((e) => {
                return e.fileType === data.doc_id;
              });
              return (
