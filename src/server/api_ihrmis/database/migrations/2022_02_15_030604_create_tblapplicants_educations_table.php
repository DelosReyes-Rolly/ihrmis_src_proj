...
class CreateTblapplicantsEducationsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('tblapplicants_educations', function (Blueprint $table) {
            ...
            $table->integer('edu_app_idref', true);
            $table->timestamp('edu_app_time')->useCurrent()->index('edu_app_time')->comment('Timestamp or date and time the record is created. Served as a secondary key for a unique identification of records.');
            ...
            $table->index(['edu_app_id', 'edu_app_time'], 'edu_app_id');
        });
    }
