...
class JvscrwService {
  ...
  private function submitCreatePdf($request, $id){
    ...
    if(!empty($prepared[2])){
      ...
      $objectSign->pre_sign = $request->pre_sign->path();
    }

    if(!empty($approved[2])){
      $objectSign->app_sign = storage_path('app/public/jvscrw/prepared/'. $approved[2]);
    } else {
      $objectSign->app_sign = $request->app_sign->path();
    }
